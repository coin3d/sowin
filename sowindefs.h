...
#define SOWIN_STUB_FUNC __PRETTY_FUNCTION__
#else
#if HAVE_VAR___FUNCTION__
#define SOWIN_STUB_FUNC __FUNCTION__
#else
#define SOWIN_STUB_FUNC ((char *)0L)
#endif
#endif

// SOWIN_STUB(): this is the method which prints out stub
...
#define SOWIN_STUB() \
  do { \
    ...
    if ( _not_null(SOWIN_STUB_FILE) ) { \
      (void)fprintf(stderr, " at %s", SOWIN_STUB_FILE); \
      if ( _not_null((void *)SOWIN_STUB_LINE) ) \
        (void)fprintf(stderr, ":line %u", SOWIN_STUB_LINE); \
      if ( _not_null(SOWIN_STUB_FUNC) ) \
        (void)fprintf(stderr, ":[%s]", SOWIN_STUB_FUNC); \
    } \
    (void)fprintf(stderr, "\n"); \
  } while (0)
